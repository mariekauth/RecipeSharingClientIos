...
class MockDBS {
    ...
    func createMockDBS(){
        ...
        let mariePBCookies = Recipe(
            title: "Peanut Butter Cookies",
            description: nil,
            instruction: "Cream together butter, peanut butter and sugars. Beat in eggs.\r\rIn a separate bowl, sift together flour, baking powder, baking soda, and salt. Stir into batter. Put batter in refrigerator for 1 hour.\r\rRoll into balls and put on baking sheets. Flatten each ball with a fork, making a criss-cross pattern. Bake in a preheated 375º oven for about 10 minutes or until cookies begin to brown. Do not overbake.\r\rBake at 375º for 10 minutes\r\r",
            ingredients: [
                "1 cup butter",
                "1 cup peanut butter",
                "1 cup sugar",
        ...
        let marieBake = RecipeBook(
            title: "Marie Bake Book",
            user: UserConstant.marie,
            openToPublic: true,
            recipe: [marieCheeseCake, marieThumbprintCookies, mariePBCookies]
        )
        ...
        let patrickCabbageRolls = Recipe(
            title: "Cabbage Rolls",
            description: nil,
            instruction: "Mix Hamburger, Rice, Eggs, Onion, Ketchup, Salt, Pepper, and Sugar\r\rSimmer Cabbage Upside Down, holding by the stem. As leaves soften cut away from stem using a sharp knife and remove from pan. Lay them on a cookie sheet.\r\rRoll meat mix in loosely in cabbage leaves, folding both sides of leaves in as you roll. Put end of roll down, or hold with toothpicks.\r\rWarning: Rolling tightly will cause hard rock like balls, do NOT do it.\r\rPut in Roasting Pan and pour tomato juice over the top\r\rBake 3 hours at 350°\r\r",
            ingredients: [
                "1 Head Cabbage",
        ...
        let patrickDinner = RecipeBook(
            title: "Patrick Dinner Book",
            user: UserConstant.patrick,
            openToPublic: true,
            recipe: [patrickCabbageRolls, patrickPorkChops]
        )
        
        users.append(patrickDinner)
    }
    
    func filterByUser(userName: String) {
        for book in users {
            if book.user == userName {
                userLogged.append(book)
            }else{
                if book.openToPublic == true, book.user != userName, book.user != nil, userName != ""{
                    userLogged.append(book)
                }
            }
        }
